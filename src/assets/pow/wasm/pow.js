  ...
  if (typeof console !== 'undefined') {
    if (!Module['print']) {
      Module['print'] = function shell_print(x) {
        document.getElementById('wasmWork').value = 'Calculated ' + x + ' frames...';
      };
    }
    if (!Module['printErr']) {
...
var Runtime = {
  ...
  makeBigInt: (function(low, high, unsigned) {
    const ret = unsigned ? +(low >>> 0) + +(high >>> 0) * 4294967296 : +(low >>> 0) + +(high | 0) * 4294967296;
    return ret;
  }),
  GLOBAL_BASE: 1024,
  QUANTUM_SIZE: 4,
...
function allocate(slab, types, allocator, ptr) {
  ...
  let i = 0;
  let type; let typeSize; let previousType;
  while (i < size) {
    let curr = slab[i];
...
function UTF8ArrayToString(u8Array, idx) {
  ...
  if (endPtr - idx > 16 && u8Array.subarray && UTF8Decoder) {
    ...
    let u0; let u1; let u2; let u3; let u4; let u5;
    let str = '';
    while (1) {
      u0 = u8Array[idx++];
      if (!u0) return str;
...
let HEAP; let buffer; let HEAP8; let HEAPU8; let HEAP16; let HEAPU16; let HEAP32; let HEAPU32; let HEAPF32; let HEAPF64;
...
let STATIC_BASE; let STATICTOP; let staticSealed;
let STACK_BASE; let STACKTOP; let STACK_MAX;
let DYNAMIC_BASE; let DYNAMICTOP_PTR;
...
Math.imul = Math['imul'];
if (!Math['fround']) {
  const froundBuffer = new Float32Array(1);
...
function integrateWasmJS() {
  const method = Module['wasmJSMethod'] || 'native-wasm';
  Module['wasmJSMethod'] = method;
  let wasmTextFile = Module['wasmTextFile'] || 'pow.wast';
  let wasmBinaryFile = Module['wasmBinaryFile'] || 'pow.wasm';
  let asmjsCodeFile = Module['asmjsCodeFile'] || 'pow.temp.asm.js';
  ...
  const methodHandler = Module['asm'];
}
integrateWasmJS();
const ASM_CONSTS = [];
...
__ATINIT__.push();
memoryInitializer = Module['wasmJSMethod'].indexOf('asmjs') >= 0 || Module['wasmJSMethod'].indexOf('interpret-asm2wasm') >= 0 ? 'pow.js.mem' : null;
const STATIC_BUMP = 6032;
...
const _launchPoW = Module['_launchPoW'] = (function() {
  return Module['asm']['_launchPoW'].apply(null, arguments);
});
...
const _llvm_bswap_i32 = Module['_llvm_bswap_i32'] = (function() {
